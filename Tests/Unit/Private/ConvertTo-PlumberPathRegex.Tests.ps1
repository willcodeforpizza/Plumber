...
Describe 'ConvertTo-PlumberPathRegex' {
    It 'pins the regex for <Pattern> matches <SamplePath>=<Expected>' -ForEach @(
        ...
        InModuleScope Plumber -Parameters @{Pattern = $Pattern; SamplePath = $SamplePath; Expected = $Expected} {
            $regex = ConvertTo-PlumberPathRegex -Pattern $Pattern
            ($SamplePath -match $regex) | Should -Be $Expected -Because "Pattern '$Pattern' against '$SamplePath' should produce $Expected (regex: $regex)"
        }
    }
}

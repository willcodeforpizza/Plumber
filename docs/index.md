# Plumber

![Plumber banner](images/Plumber-banner.png)

Shared Invoke-Build tasks for PowerShell validation pipelines.

## Documentation

- [Configuration](configuration.md)
- [Task index](tasks/index.md)
- [Writing tasks](writing-tasks.md)
- [Local tasks](local-tasks.md)
- [Invoke-Plumber JSON output contract](json-output.md)
- [Architecture decisions](adr/001-record-architecture-decisions.md)
- [Repository README](https://github.com/willcodeforpizza/Plumber#readme)

## Task Groups

- [Validate](tasks/Validate.md)
- [CodeQuality](tasks/CodeQuality.md)
- [Content](tasks/Content.md)
- [ModuleConventions](tasks/ModuleConventions.md)
- [ReleaseHygiene](tasks/ReleaseHygiene.md)
